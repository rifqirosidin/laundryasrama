		...
		<div class="art-layout-cell art-content">
			<div class="art-post">
				...
				<!-- MULAI KONTEN -->
							...
							<fieldset><legend>Grafik Cuci Kiloan</legend><br/>
							<form name="g_form" method="post" action="<?php echo $g_form; ?>" accept-charset="utf-8">
								<table width='55%'>
									...
									<?php foreach ($dbarea->result() as $row){ ?>
												<option value="<?php echo $row->id_area; ?>" class="<?php echo $row->id_city; ?>"><?php echo $row->area_name; ?></option>
									<?php } ?>
											</select>
										</td>
									</tr>
...
$(function () {
	...
	$(document).ready(function() {
		chart = new Highcharts.Chart({
			...
			title: {
				text: 'Grafik Cuci Kiloan <?php echo $wilayah; ?>',
				x: -20 //center
			},
			xAxis: [{
					type: 'datetime',
			...
			<?php
					for($a=0;$a<sizeOf($db['date']);$a++){
						if($a == 0){
							...
							echo 'xAxis: 0,';
							echo 'data: [';
							echo '[Date.parse("'.$db['date'][$a].'"), '.$db['jumHari'][$a].']';
							echo ']}';
						}else{
							echo ',{name: \''.$db['area'][$a].'\',';
							echo 'type: \'column\',';
						...
						data: [
							<?php
								for($a=0;$a<sizeOf($db['cuci']);$a++){
									...
									if($a == 0)
										echo '[Date.parse("'.$db['tgl'][$a].'"), '.$count.']';
									else
										echo ',[Date.parse("'.$db['tgl'][$a].'"), '.$count.']';
								}
							?>
