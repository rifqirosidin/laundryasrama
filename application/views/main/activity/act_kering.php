			...
			<div class="art-post">
				...
				<!-- MULAI KONTEN -->
						<div class="art-postcontent">
							...
							<fieldset>
								<div class="list">PENGERINGAN</div><br/>
<?php if ($error == 'NIP') {?>
...
<?php if ($error == 'ASET') {?>
	<div class="attention" style="width:95%;">
		<b>MAAF, KODE MESIN PENGERING YANG ANDA MASUKKAN SALAH!</b>
	</div>
<?php } ?>
<?php if ($error == 'YES') {?>
	<div class="attention" style="width:95%;">
							...
							<?php $i=1; foreach($dataCeklist as $rowcek){ ?>
									<tr style="font-weight:bold;">
										...
										<td>
											...
											<input type="hidden" name="status<?php echo $i; ?>" value="<?php echo $rowcek->stat_kering; ?>"/>
									<?php if($rowcek->stat_kering == "Sudah"){ ?>
											<input type="hidden" name="mesin<?php echo $i; ?>" value="<?php echo $rowcek->aset_kering; ?>"/><?php echo $rowcek->aset_kering; ?>
									<?php }else{ ?>
											<select name="mesin<?php echo $i; ?>">
												<option value="">Pilih Mesin Pengering</option>
									<?php $j=0; foreach($mkrg as $bot){ ?>
												<option value="<?php echo $bot->kode_aset; ?>">Mesin Pengering <?php echo ++$j; ?></option>
									<?php } ?>
											</select>
									<?php } ?>
